...
class ProjectListCreateAPIView(APIView):
    ...
    @extend_schema(
        request=ProjectCreateSerializer, responses={status.HTTP_201_CREATED: ProjectCreationResponseSerializer}
    )
    def post(self, request: Request) -> Response:
        ...
        return Response(
            ProjectCreationResponseSerializer(result).data,
...
class ProjectDetailAPIView(APIView):
    ...
    def get(self, request: Request, project_id: UUID | str) -> Response:
        try:
            project = get_project_for_organization(organization=current_organization(request), project_id=project_id)
        except ProjectError as error:
            raise_project_error(error)
...
class ProjectEnvironmentListCreateAPIView(APIView):
    ...
    @extend_schema(
        responses=EnvironmentSerializer(many=True),
    )
    def get(
        self,
        request: Request,
        ...
        environments = environments_for_project(
            organization=organization,
            project=project,
        )
    ...
    @extend_schema(
        request=EnvironmentCreateSerializer,
        responses={status.HTTP_201_CREATED: EnvironmentSerializer},
    )
    def post(
        self,
        request: Request,
...
class EnvironmentDetailAPIView(APIView):
    ...
    def patch(self, request: Request, environment_id: UUID | str) -> Response:
        ...
        try:
            existing_environment = get_environment_for_organization(
                organization=organization,
                environment_id=environment_id,
            )
            ...
            environment = update_environment(
                actor=authenticated_user(request),
                organization=organization,
                project_id=existing_environment.project_id,
                environment_id=environment_id,
                name=serializer.validated_data.get("name"),
                environment_type=serializer.validated_data.get("environment_type"),
                status=serializer.validated_data.get("status"),
                capture_mode_override_supplied=("capture_mode_override" in serializer.validated_data),
                capture_mode_override=serializer.validated_data.get("capture_mode_override"),
                retention_days_override_supplied=("retention_days_override" in serializer.validated_data),
                retention_days_override=serializer.validated_data.get("retention_days_override"),
                audit_context=audit_context_from_request(request),
            )
        except ProjectError as error:
